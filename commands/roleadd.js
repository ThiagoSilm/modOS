...
module.exports = {
  ...
  execute(message, args) {
    ...
    if (member && cargo) {
      member.roles.add(cargo).then(message.channel.send(embed)).catch(err => {
        message.channel.send(`${message.author} o cargo que você mencionou é maior que o meu`)
      });

      const embed = new Discord.MessageEmbed()
        .setColor(coloRole())
        .setTitle('<a:checkyes:738832483076014191> Cargo adicionado com sucesso')
        .setDescription(`o cargo ${cargo} foi adicionado ao usuario`);
    }
